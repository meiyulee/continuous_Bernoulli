...
void moment_X_bar(int n,double *datastore)
{
	int i;
	...
	if (sumX2>0) 
	{
		...
		printf(" E(X bar)=%20.10f,\n Var(X bar)=%20.10f,\n",EX,VarX);
        printf(" skewed coefficient(X bar)=%20.10f,\n kurtosis coefficient(X bar)=%20.10f\n",skew,kurtosis);
      }
      else
       printf(" E(X bar)=%20.10f\n",EX);
}
...
{  
    ...
    double *datastore,lamda,tepdata;
    char *ss;
