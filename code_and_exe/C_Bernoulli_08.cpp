...
double goodness_of_fitX(int n,double *data,int *dfret,double lamda,double *ret_pvalue,double *datastore)
{
  ...
  *dfret=classx-2; 
  ...
  printf(" H0:X~Continuous Bernoulli(lamda),lamda unknown\n");
  printf(" H1:against H0,\n");
  printf(" lamda estimated=%f\n",lamda);
...
int main() {  
     ...
     double *sampledata,tep,lamdahat,samplemean,pvalue,*datastoreX;
	 ...
	 FILE *f;
	  ...
	  if (lamdahat<0) { printf(" the lamda hat is error, this program will be terminated.\n"); goto end;}
	  printf(" the sample mean=%20.10f\n the lamda estimated=%f.\n",samplemean,lamdahat);
	  // the goodness of fit
      goodness_of_fitX(samplesize,sampledata,&df,lamdahat,&pvalue,datastoreX);
      end:
	  system("pause");
